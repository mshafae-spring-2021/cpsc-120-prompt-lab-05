...
/// SimpleValueToFaceValueString takes the simple_value of a card and returns
/// the face_value.
...
/// simple_base_value and the steps.
///
/// Since the simple_base_value of the base card is an integer between 0
/// and 12, we can add steps to the simple_base_value to find the simple value
/// of the secret card. To wrap the count around (from K to A), use modulo 13.
...
/// second, the function returns the rank of a given suit. Suits are ranked in
/// alphabetical order (C=1, D, H, S).
...
/// Given a two to three letter string that represents a card, return a single
/// letter string that represents the card's suit
...
/// Given a two to three letter string that represents a card, return a single
/// letter string that represents the card's face_value
/// \param card A two to three letter string representing a card,
/// ex. AS, 4D, 10H, QS
...
/// card. The remaining three cards are card_one (argv[2]), card_two (arg[3]),
/// card_three (argv[4]).
