...
/// SimpleValue takes the face_value of a card as a string
/// and returns the simple_value of the card as an int
/// \param face_value A string representing the value of a card (A, 2, ..., Q, K)
...
// TODO: Write a function named SimpleValueToFaceValueString; the function returns a
// string and takes an int as a parameter
/// SimpleValueToFaceValueString takes the simple_value of a card and returns the 
/// face_value.
/// \param simple_value An int representing the value of a card as an int (0, 1, ..., 11, 12)
...
// ints as parameters
/// SecretCardValue returns the face_value of the secret card given the simple_base_value
/// and the steps.
/// \param simple_base_value The simple_value of the base card, for example 3C -> 2
/// \param steps The number of steps from the base simple_value to the secret card value
...
/// The offset of each suit; since the cards are ordered value first, suit second, the function
/// returns the rank of a given suit. Suits are ranked in alphabetical order (C=1, D, H, S).
...
/// Given a two to three letter string that represents a card, return a single letter string
/// that represents the card's suit
/// \param card A two to three letter string representing a card, ex. AS, 4D, 10H, QS
/// \returns A single letter string which has the suit of the given card
string Suit(const string& card) {}

/// Given a two to three letter string that represents a card, return a single letter string
/// that represents the card's face_value
/// \param card A two to three letter string representing a card, ex. AS, 4D, 10H, QS
/// \returns A single letter string that which has the face_value of the
/// card (A, 2, 3, 4, 5, 6, 7, 8, 9, 1, J, Q, K)
/// \remark Although a 10 has two letters, only the first letter is needed since it is
/// the only card that has a value that starts with 1.
...
/// SecretSteps calculates the number of steps that are being signaled given the order of 
/// the three cards.
///
/// There are four cards given on the command line. The first card is the base card. The
/// remaining three cards are card_one (argv[2]), card_two (arg[3]), card_three (argv[4]).
/// Given the relative order of these cards, determine the steps to return (labeled Value to
/// Add in the table).
///
/// | Order | Value to Add |
/// | :--- | :---: |
...
/// \remark Must have at least 5 arguments. Each argument represents a card.
